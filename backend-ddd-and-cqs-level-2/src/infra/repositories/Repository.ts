...
@injectable()
export class Repository<TDomainEntity> implements IRepository<TDomainEntity> {
  ...
  /*async findBy(parameter: any): Promise<TDomainEntity | null> {
    const dbResult = await this.collectionInstance.findOne({ parameter })
    if (!dbResult) return null
    return this.dataMapper.toDomain(dbResult)
  }*/
  ...
  async save(entity: TDomainEntity): Promise<void> {
    ...
    if (!exists) {
      await this.collectionInstance.insertOne(
        this.dataMapper.toDalEntity(entity)
      )
      return
    }
